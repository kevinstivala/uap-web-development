...
export const ActionButtonList = ({ sendHash, sendSignMsg, sendBalance }: ActionButtonListProps) => {
  ...
  return (
    <div className="card info-actions" aria-label="Acciones de la wallet">
    {isConnected ? (
      <div className="btn-row">
        <button className="secondary" onClick={() => open()} title="Abrir AppKit">Abrir</button>
        <button className="secondary" onClick={handleDisconnect}>Desconectar</button>
        <button className="secondary" onClick={() => switchNetwork(networks[1]) }>Cambiar red</button>
        <button className="secondary" onClick={handleSignMsg}>Firmar mensaje</button>
        <button className="primary" onClick={handleSendTx}>Enviar transacción</button>
        <button className="secondary" onClick={handleGetBalance}>Ver balance</button>
      </div>
    ) : (
      <div className="muted">Conecta tu wallet para ver acciones adicionales</div>
    )}
  </div>
  )
}
