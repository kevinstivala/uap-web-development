...
export class TaskController {
  ...
  getTasks = async (req: Request, res: Response) => {
    ...
    try {
      ...
      res.json(
        result.tasks.length ? result : { tasks: [], total: 0, limit, offset }
      );
    } catch (error: any) {
  ...
  addTask = async (req: Request, res: Response) => {
    ...
    if (!text || !boardId)
      return res
        .status(400)
        .json({ error: "Datos requeridos (text o boardId)" });
    try {
  ...
  deleteTask = async (req: Request, res: Response) => {
    ...
    if (!boardId) return res.status(400).json({ error: "boardId requerido" });
    try {
      await this.TaskService.deleteTask(id, String(boardId));
      res.status(204).send();
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  deleteCompletedTasks = async (req: Request, res: Response) => {
    ...
    if (!boardId) return res.status(400).json({ error: "boardId requerido" });
    try {
      const result = await this.TaskService.deleteCompletedTasks(String(boardId));
